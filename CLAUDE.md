...
- **Detection model** (`src/headbang.js`): all three axes (pitch, yaw, roll) are tracked. Each raw angle is **unwrapped** (survives ±180° matrix wrap) and **one-euro filtered** (adaptive low-pass: smooths jitter at rest, stays transparent during fast bangs). Reversals are found by **peak detection** — a turning point is confirmed once the angle retreats `SWING_HYSTERESIS_DEG` from the running extreme, and the "swing" is the true peak-to-peak travel (measuring real amplitude, unlike velocity-threshold crossings). Amplitude/peak-speed/tempo (median of trimmed reversal intervals) over a 2 s window produce a smoothed 0–100 intensity score mapped onto `TIERS`. The dominant axis (weighted amplitude, pitch-biased) drives style classification (up-down / side-to-side / tilt / windmill) and counting: a bang is a negative-direction swing ≥ `BANG_MIN_DEGREES` at ≥ `BANG_MIN_SPEED`, with a global refractory. The banner uses score hysteresis (`DETECT_ON_SCORE`/`DETECT_OFF_SCORE` + hold). The danger tier is judged from physical motion only (amplitude + tempo, never the score, so it decays cleanly) and reflects the BMJ (2008) ~75°-at-metal-tempo heuristic.
- **Cosmic theme.** The UI matches eelkedevries.com: near-black canvas, gold (`#e8b43a`) and iris-blue (`#6ba6e4`) accents, starfield/nebula background, Spectral/Lexend/JetBrains Mono fonts, a masthead linking back to the site. Semantic green (`#74c081`) / soft-red (`#e57a7a`) / alarm-red (`#ff5c5c`) mark the detected / idle / danger states; the gauge zone ramp runs cool→warm (grey → iris → teal → gold → orange → red).
